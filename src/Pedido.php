...
class Pedido
{
    ...
    {
        $sql = "INSERT INTO `detalle_pedidos`(`pedidos_id`, `bebidas_id`, `precio`, `cantidad`) 
        VALUES (:pedidos_id,:bebidas_id,:precio,:cantidad)";

        $resultado = $this->cn->prepare($sql);

        $_array = array(
            ":pedidos_id" => $_params['pedidos_id'],
            ":bebidas_id" => $_params['bebidas_id'],
    ...
    public function mostrarDetallePorIdPedido($id)
    {
        ...
        INNER JOIN bebidas be ON be.id= dp.bebidas_id
